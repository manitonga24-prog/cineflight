...
def _ancrer_et_optimiser(pto, log):
    ...
    # ⚠⚠ NE PAS RÉESSAYER « LACET SEUL » SANS PLUS DE CONNAISSANCE (essayé le 2026-07-29).
    # Variante éprouvée : `["v y%d" % n]` — n'optimiser que le cap, en laissant
    # l'inclinaison et le roulis de la nacelle intacts. Le raisonnement était bon (seul le
    # cap est bruité, la nacelle est précise au dixième de degré) et le RÉSULTAT était
    # désastreux : dominante magenta sur toute l'image et sphère repartie en « petite
    # planète ». `autooptimiser -n` fait donc quelque chose aux paramètres photométriques
    # que la simple liste de variables géométriques ne suffit pas à contrôler — cause NON
    # établie. Revenu en arrière le même jour.
    ...
    try:
        ...
        if angles is not None:
            ...
            if _ecrire_positions(avec_opt, angles, log):
                ...
                # ⚠ DEUX ESSAIS POUR LA METTRE EN TETE ONT ECHOUE (2026-07-28 et 07-29) :
                # trois axes libres -> 27,8 % de ciel comble ; lacet seul -> dominante
                # magenta. Elle NE DOIT PAS repasser en premier sans une mesure qui
                # l'appuie. Le dedoublement des sentiers reste ouvert.
